...
class move_x(Node):
    ...
    def timer_callback(self):
        ...
        # if ball_y != 0:  # 持续更新y坐标
        #     self.y_rec.pop(0)
        #     self.y_rec.append(ball_y)
        avx=sum(self.x_rec)/len(self.x_rec)  # 取一段时间的ballx坐标平均值，作为ballx的真正位置
        # avy=sum(self.y_rec)/len(self.y_rec)  # 取一段时间的bally坐标平均值，作为bally的真正位置
        self.xx.pop(0) # self.xx记录球的连续三个x值
        self.xx.append(avx)
        # self.yy.pop(0) # self.yy记录球的连续三个y值
        # self.yy.append(avy)
        ...
        if (size > 20) & (size < 100): # 如果球很远或者走掉了
            if  avx < 300: #球从左侧溜走:向左走  320是像素中心位置
                self.speed_x, self.speed_y, self.speed_z = 0.0, 0.2, 0.0
            elif avx > 340: # 球从右侧溜走：向右走
                self.speed_x, self.speed_y, self.speed_z = 0.0, -0.2, 0.0
            else:
                self.speed_x = 0.0
        ...
        if size >= 100: # 如果球很近了
            ...
            if avx > 300 and avx < 340:  #球在视野中心：不再平移
                self.speed_x = 0.0
                self.aim = True
            else:
                if (self.x_rec[0]+3.0 < self.x_rec[1]) & (self.x_rec[1]+3.0 < self.x_rec[2]): # 如果x越来越大，狗往右走
                    self.speed_x, self.speed_y, self.speed_z = 0.0, -0.27, 0.0
                elif (self.x_rec[2]+3.0 < self.x_rec[1]) & (self.x_rec[1]+3.0 < self.x_rec[0]):# 如果x越来越小，狗往左走
                    self.speed_x, self.speed_y, self.speed_z = 0.0, 0.27, 0.0
                else: # 如果球几乎不动，狗不动
                    self.speed_x = 0.0
                    self.aim = True
        ...
        self.get_logger().info(f"x={ball_x},origin_x={self.x_rec},stable_x={self.xx},move_x={self.speed_x}")
