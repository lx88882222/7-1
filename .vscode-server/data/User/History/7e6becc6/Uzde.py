...
class Location():
    ...
    def get_data(self):
        ...
        self.ball = (float(parts[0]) if parts[0] != 'None' else None, float(parts[1]) if parts[1] != 'None' else None)
        self.red_dog = (float(parts[2]) if parts[2] != 'None' else None, float(parts[3]) if parts[3] != 'None' else None)
        self.black_dog = (float(parts[4]) if parts[4] != 'None' else None, float(parts[5]) if parts[5] != 'None' else None)
    ...
    def get_ball_loc(self):
        self.get_data()
        loc = [self.ball_loc_rec[1],self.ball_loc_rec[2]]
        return loc
    def get_red_loc(self):
        self.get_data()
        loc = [self.red_dog_loc_rec[1],self.red_dog_loc_rec[2]]
        return loc
    def get_black_loc(self):
        self.get_data()
        loc = [self.black_dog_loc_rec[1],self.black_dog_loc_rec[2]]
        return loc
    def __del__(self):
