...
class move(Node):
    ...
    def goto(self,target,frequency = 0.1):
        '''
        goto(self,target:target_coords)
        go to point target at max speed
        '''
        while not self.location.in_place(target):
            my_loc = self.location.black_dog_coords()   #   TODO red dog should chang tihs line
            vel = [list(self.max_vel(target,my_loc))+[.0]]
            self.go(vel)
            time.sleep(frequency)
        print(f'targeting {target}, arrived at {self.location.black_dog_coords}') 
        return True
