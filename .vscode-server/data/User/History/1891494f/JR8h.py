...
class move_x(Node):
    ...
    def timer_callback(self):
        ...
        if (size > 20) & (size < 100): # 如果球很远或者球走掉了
        ...
        if size >= 100:
            ...
            if avx > 300 and avx < 340:  #球在视野中心：不再平移
                ...
                if (self.xx[0]+20.0 < self.xx[1]) & (self.xx[1]+20.0 < self.xx[2]): # 如果x越来越大，狗往右走
                    self.speed_x, self.speed_y, self.speed_z = 0.0, -0.27, 0.0
                elif (self.xx[2]+20.0 < self.xx[1]) & (self.xx[1]+20.0 < self.xx[0]):# 如果x越来越小，狗往左走
                    self.speed_x, self.speed_y, self.speed_z = 0.0, 0.27, 0.0
                else: # 如果球几乎不动，狗不动
                    self.speed_x = 0.0
                    self.aim = True
        ...
        self.get_logger().info(f"x={ball_x},arr={self.x_rec},arr={self.xx},move_x={self.speed_x}")
